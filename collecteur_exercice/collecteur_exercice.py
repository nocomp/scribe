...
async def save_scenario(request:Request, auth=Depends(require_auth)):
    ...
    return {"ok":True,"filename":filename,"id":sid}
...
async def generate_scenario_ia(body:GenRequest, auth=Depends(require_auth)):
    ...
    try:
        ...
        return {"ok":True,"scenario":scenario,"filename":f"{sid}.json"}
    except json.JSONDecodeError as e:
...
# v2.3.101 — Reset complet d'exercice. Efface tout sur toutes les instances
