...
def _active_incidents(etab: dict) -> list:
    """Incidents non résolus de l'établissement."""
    out = []
    for inc in (etab.get("incidents") or []):
        if isinstance(inc, dict):
            status = (inc.get("status") or "").upper()
            if status not in ("RÉSOLU", "RESOLU", "ARCHIVE", "ARCHIVÉ"):
                out.append(inc)
    return out


def _critiques(incidents: list) -> list:
    return [i for i in incidents if (i.get("urgency") or 1) >= 3]
...
def _has_plan_blanc(etab: dict) -> bool:
    """L'établissement a-t-il activé un Plan Blanc (déclaration ou décision) ?"""
    # 1. Déclaration de type plan_blanc
    for d in (etab.get("declarations") or []):
        if not isinstance(d, dict):
            continue
        tc = (d.get("type_crise") or "").lower()
        if "plan" in tc and "blanc" in tc:
            return True
    # 2. Indication via niveau de tension 3 (crise = plan blanc implicite)
    for d in (etab.get("declarations") or []):
        if not isinstance(d, dict):
            continue
