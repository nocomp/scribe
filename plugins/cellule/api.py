...
def create_decision(dec: DecisionCreate, db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(new_dec)
    # v2305 — Notifier toute la cellule quand une décision formelle est prise.
    # Urgency 3 (alerte) car une décision de crise est par nature un
    # événement important qu'il faut porter à la connaissance de tous les
    # membres de la cellule. Fail-safe.
    try:
        from plugins.notifications.dispatcher import notify_sync
        contenu = (getattr(new_dec, 'contenu', '') or '')[:150]
        responsable = getattr(new_dec, 'responsable', '') or ''
        notify_sync(
            event_type="decision_taken",
            title=f"📋 Décision de crise prise",
            body=f"{contenu}" + (f" — par {responsable}" if responsable else ""),
            urgency=3,
            context={
                "decision_id": new_dec.id,
                "url": f"/#cellule/decisions/{new_dec.id}",
            },
        )
    except Exception:
        pass
