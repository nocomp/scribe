...
def create_transfert(body: TransfertCreate, db: Session = Depends(get_db),
    ...
    # v2305 — Notifier la cellule de crise de tout nouveau transfert.
    # Urgency par défaut 2 (vigilance) car un transfert est en soi un
    # événement à suivre mais rarement critique (sauf motif médical grave
    # que le dispatcher ne peut pas deviner depuis ce contexte).
    try:
        from plugins.notifications.dispatcher import notify_sync
        dest = getattr(t, 'etablissement_destination', '') or '?'
        origine = getattr(t, 'etablissement_origine', '') or '?'
        motif = (getattr(t, 'motif', '') or '')[:100]
        notify_sync(
            event_type="transfert_created",
            title=f"🚑 Transfert {origine} → {dest}",
            body=f"{motif}" if motif else "Nouveau transfert patient créé",
            urgency=2,
            context={
                "transfert_id": t.id,
                "url": f"/#transferts/{t.id}",
            },
        )
    except Exception:
        pass  # fail-safe : un échec notif ne doit pas bloquer la création
    return _serialize(t)
