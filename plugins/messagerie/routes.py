...
def _msg_upload_policy(fichiers) -> int:
    ...
    (octets). Lève HTTPException 415 si une extension n'est pas autorisée."""
    eff_max = MAX_ATTACHMENT_SIZE
    for up in (fichiers or []):
        try:
            from app.plugin_settings import enforce as _enf
            eff_max = _enf("messagerie", getattr(up, "filename", ""), MAX_ATTACHMENT_SIZE)
...
def _msg_max_attachments() -> int:
    """Nombre max de PJ par message (config admin plugin, sinon défaut)."""
    try:
        from app.plugin_settings import get_plugin_config
        n = int(get_plugin_config("messagerie").get("max_attachments") or 0)
        return n if n > 0 else MAX_ATTACHMENTS_PER_MSG
    except Exception:
...
async def create_message(
    ...
    for d in dest_list:
        ...
        if d.get("type") == "user":
            ...
            _sigle = str(d.get("value") or "").strip().upper()
            if _sigle:
...
async def _deliver_to_supervision(msg, db):
    ...
    if db is not None and msg.expediteur_id:
        try:
            _su = db.query(User).filter(User.id == msg.expediteur_id).first()
            if _su:
...
def reply_message(
    ...
    if src.expediteur_id and src.expediteur_id != user.id:
        # Message interne : expéditeur est un user local
    ...
    elif not src.expediteur_id and src.expediteur_addr:
        # Message inter-GHT : répondre à l'AGENT d'origine si son username a été
        # préservé à l'ingestion (sinon, repli sur l'établissement). Sans cela, la
        # réponse partait vers le seul sigle et n'atteignait pas l'expéditeur.
        sigle = (src.expediteur_addr or "").strip()
        origin_user = ""
        try:
            meta = src.backend_meta or {}
            if isinstance(meta, dict):
                origin_user = (meta.get("origin_username") or "").strip()
        except Exception:
            origin_user = ""
        if sigle and origin_user:
            disp = (src.expediteur_nom or origin_user)
            dest.append({"type": "agent_federe", "value": origin_user,
...
def download_attachment(
    ...
    # Auth : en-tête Authorization OU ?token= en query. Les liens de PJ ouverts
    # directement dans le navigateur (supervision, nouvel onglet, copier-coller)
    # n'envoient pas l'en-tête Bearer → on accepte le token en query en repli.
...
# Politique d'upload LOCALE : poids max par PJ + types de fichiers autorisés
# (catégories MIME) + nombre max de PJ. Repli sur la politique centrale.
@router.get("/admin/config")
def messagerie_admin_config_get(admin=Depends(require_admin)):
    from app.plugin_settings import get_plugin_config, categories_meta
    return {"config": get_plugin_config("messagerie"), "categories": categories_meta()}


@router.post("/admin/config")
def messagerie_admin_config_post(payload: dict, admin=Depends(require_admin)):
