...
def get_upload_policy() -> dict:
    ...
    Renvoie {max_bytes, allowed_exts}. max_bytes=0 → pas de limite imposée
    (le module appelant garde son défaut). allowed_exts=[] → toutes extensions OK."""
    d = get_domain("uploads")
    try:
        max_mb = float(d.get("max_size_mb") or 0)
    except Exception:
        max_mb = 0
    exts = d.get("allowed_extensions") or []
    ...
    for e in exts:
        ...
        if e:
            norm.append("." + e)
    return {"max_bytes": int(max_mb * 1024 * 1024) if max_mb > 0 else 0,
            "allowed_exts": norm}


def enforce_upload_policy(filename: str, default_max_bytes: int) -> int:
    """Applique la politique : lève HTTPException 415 si l'extension n'est pas dans
    la liste blanche (quand une liste est configurée), et renvoie la taille max
    effective en octets (politique centrale si définie, sinon le défaut fourni).
    Tolérant aux pannes : en cas d'erreur, renvoie le défaut sans bloquer."""
    try:
        pol = get_upload_policy()
    except Exception:
        return default_max_bytes
    exts = pol.get("allowed_exts") or []
    if exts:
...
<<<<<<< HEAD
        ...
        try:
            from plugins.notifications.api import materialize_central_channels
