...
class FederationConfig:
    ...
    def _load(self):
        # 1. Charger depuis config.js (établissement + fédération par défaut)
        ...
        if os.path.exists(config_js):
            try:
                ...
                logger.warning(f"Federation config.js non chargée : {e}")

        # 2. Surcharger avec config.xml si présent (priorité — modifié via Admin)
        config_xml = os.environ.get("SCRIBE_CONFIG_FILE", "config.xml")
        if os.path.exists(config_xml):
            try:
                import xml.etree.ElementTree as _ET
                root = _ET.parse(config_xml).getroot()
                fed = root.find("federation")
                if fed is not None:
                    def _tx(tag, default=None):
                        el = fed.find(tag)
                        return el.text.strip() if el is not None and el.text else default
                    enabled_val = _tx("enabled", "")
                    if enabled_val:
                        self.enabled = enabled_val.lower() == "true"
                    url_val = _tx("collecteur_url", "")
                    if url_val:
                        self.collecteur_url = url_val
                    tok_val = _tx("token", "")
                    if tok_val:
                        self.token = tok_val
                    inter_val = _tx("intervalle_secondes", "")
                    if inter_val:
                        self.intervalle = int(inter_val)
            except Exception as e:
                logger.warning(f"Federation config.xml non chargée : {e}")
...
def reload_fed_config() -> FederationConfig:
    """Recharge la config depuis disk (config.xml prioritaire) sans redémarrage."""
    global _fed_config
    _fed_config = FederationConfig()
    logger.info("Federation config rechargée à chaud")
    return _fed_config
...
@router.post("/reload")
def federation_reload(user=Depends(lambda: None)):
...
def federation_pause():
    """Suspend la synchronisation (push) vers le collecteur."""
...
def federation_resume():
    ...
    reload_fed_config()  # Recharger la config en même temps
